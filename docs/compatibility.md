...
- `get_stock_prices` falls back to Nasdaq ETF historical quotes when DefiLlama returns a provider 404, so ETF benchmarks such as SPY are supported. Other provider errors still propagate. Truncated Nasdaq responses are rejected. It returns ascending time order and aggregates day bars into week/month/year locally; `time` is the bar end date in UTC.
