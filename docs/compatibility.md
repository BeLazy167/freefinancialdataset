...
- `/macro/interest-rates` reads each bank's current rate from its own publication (Fed target-range midpoint, ECB deposit facility rate, BOE Bank Rate, BOJ policy rate) with `date` as the decision date. The Bank of Japan publishes each decision only as a PDF statement, so its rate takes two Monid calls: the year's releases listing supplies the newest Statement on Monetary Policy and its date, and Context.dev's extractor reads the rate out of that PDF. `bank`, `start_date` and `end_date` all narrow the result. The pages carry full decision histories (56 Fed decisions back to 2003, 35 ECB back to 2000) from the same single scrape, and the route reports the rate in force at the start of each month, defaulting to the last twelve. `/macro/interest-rates/banks` lists only those four banks. A bank whose page cannot be read is omitted; if none can, the call answers `upstream_error`.
...
- `get_stock_prices` falls back to Nasdaq ETF historical quotes when DefiLlama returns a provider 404, so ETF benchmarks such as SPY are supported. Other provider errors still propagate. Truncated Nasdaq responses are rejected. If Nasdaq contains explicit missing OHLCV values in the requested range, the server replaces the series with Yahoo Finance daily history through Monid. The fallback requests five years of daily history and must cover every requested date returned by Nasdaq; insufficient coverage or malformed Yahoo bars still fail. No values are filled with zero, and providers are not mixed within the series. This adds a Yahoo provider call only when needed. It returns ascending time order and aggregates day bars into week/month/year locally; `time` is the bar end date in UTC.
...
- `get_earnings` with a ticker composes records from 10-K and 10-Q filing events only; 8-K earnings releases and foreign-issuer forms such as 20-F and 6-K are not composed. A valid filings index with no supported forms returns an empty earnings list, including the empty index DefiLlama currently returns for NBIS. No estimate lookup runs for an empty result. Without a ticker it answers the Nasdaq earnings calendar feed (five records). Records omit blocks whose period is not in the statements matrix.
